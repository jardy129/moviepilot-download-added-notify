...
1. 更新并启用插件后，确认 MoviePilot 的 `API_TOKEN` 可用。
2. 把 `scripts/qb_notify_moviepilot.sh` 放到 qBittorrent 容器/主机能访问的位置，并给执行权限。
3. 修改脚本顶部的 `MP_BASE_URL` 和 `MP_API_TOKEN`，或通过环境变量传入。
4. qBittorrent 添加种子时运行外部程序填写：

```bash
/config/qb_notify_moviepilot.sh added "%N" "%I" "%D" "%L" "%G" "%Z" "%T"
```

如果也想在完成时通知，在完成下载时运行外部程序填写：

```bash
/config/qb_notify_moviepilot.sh completed "%N" "%I" "%D" "%L" "%G" "%Z" "%T"
```

脚本会请求 MoviePilot 插件接口：

```text
/api/v1/plugin/DownloadAddedNotify/qbittorrent?apikey=API_TOKEN
```
